...
=begin
    Until now, all the methods that we defined in our classes 
    were publicly available, meaning that you could call those methods from outside of the class.

    There are certain situations when methods should only be visible to the class. 
    For example, imagine a Banking class with methods to calculate values for internal 
    transactions and operations. If these methods were available outside the class, the reliability of the data could be at risk.
...
=begin
    In the code above, the method days_lived is private and is only 
    accessible inside the class. We called it from the show method, which is public.
...
=begin
    An interesting thing to note about private Ruby methods is that 
    they cannot be called with an explicit receiver, even if that receiver is itself. 
    When we say "receiver", we mean the object that the method is being called from. 
    Even if we try to call the private method with self we will get an error.
...
=begin
    This code generates an error, because we tried to call the private method id 
    on self and the other object.
    To be able to do that without making the method public, 
    Ruby has the protected access control.
...
=begin
    ...
    just like private methods, but can be called for an object of the same
    class or subclasses.
=end
